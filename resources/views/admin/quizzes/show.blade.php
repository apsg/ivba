                    ...
                    <div class="box-body">
                        <form action="/admin/quizzes/{{$quiz->id}}" method="post">
                            ...
                            <div class="form-group row">
                                <div class="col-md-6">
                                    <label><input type="checkbox" name="is_random" value="1"
                                                  @if(old('is_random') || $quiz->is_random) checked="checked" @endif >
                                        Pytania w losowej kolejności?</label>
                                </div>
                                <div class="col-md-6">
                                    <label>
                                        <input required="required" type="number" min="0" max="100"
                                               value="{{ old('pass_threshold') ?? $quiz->pass_threshold }}"
                                               name="pass_threshold">
                    ...
                    <div class="box-body">
                        <form action="{{ url('admin/quizzes/'.$quiz->id.'/questions') }}" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="number" min="1" name="points" required="required"
                                       value="1">
                            </div>
                            ...
                            <div class="box @if(!isset($open) || $open != $question->id) collapsed-box @endif">
                                ...
                                <div class="box-body">
                                    ...
                                    <form class="" action="{{ url('admin/question/'.$question->id) }}" method="post">
                                        {{ csrf_field() }}
                                        {{ method_field('delete') }}
                                        <button class="btn btn-warning"><i class="fa fa-trash"></i> Usuń to pytanie
                                        </button>
                                    </form>
                                    @if( $question->type != \App\Question::OPEN )
                                        <h3>Odpowiedzi</h3>
                                        ...
                                        <div class="alert alert-light border-top">
                                            <h5>Dodaj kolejną odpowiedź</h5>
                                            <form action="{{ url('admin/question/'.$question->id.'/options') }}"
                                                  method="post" class="add_option_form"
                                                  data-for="options_{{ $question->id }}"
                                                  class="d-flex">
                                                {{ csrf_field() }}
                                                <div class="">
                                                ...
                                                <div class="">
                                                    <label><input type="checkbox" name="is_correct" value="1"> Poprawna
                                                    </label>
                                                </div>
                                                <div class="">
                                                    <button class="btn btn-primary"><i class="fa fa-plus"></i> Dodaj
                                                    </button>
                                                </div>
...
@push('scripts')
    <script type="text/javascript">
        ...
        function updateOrder() {
            ...
            $.post('{{ url('/admin/quizzes/'.$quiz->id.'/question_order') }}', {
                _token: '{{ csrf_token() }}',
                order: order
            })
                .done(function (r) {
